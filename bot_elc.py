...
# Получаем токен из переменных окружения (без dotenv)
BOT_TOKEN = os.environ.get('BOT_TOKEN')

if not BOT_TOKEN:
    # Вместо raise ValueError используем логирование и выход
    logging.error("❌ Токен бота не найден в переменных окружения!")
    logging.error("Добавьте BOT_TOKEN в Environment Variables BotHost")
    exit(1)
...
def main():
    ...
    # Регистрируем обработчик для кнопок меню
...
if __name__ == '__main__':
    main()        'Используйте кнопки меню ниже для получения информации:'
    )
    await update.message.reply_text(
        welcome_text,
        reply_markup=get_main_keyboard()
